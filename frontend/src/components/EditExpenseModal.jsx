...
export default function EditExpenseModal({ groupId, members, expense, onClose }) {
  const { user } = useAuth();
  ...
  const [form, setForm] = useState({
    ...
    amount: (expense.totalAmountPaisa / 100).toFixed(2),
  });

  // Pre-select existing split members by fetching splits already stored
  const [splitAmong, setSplitAmong] = useState(expense.existingSplitUserIds || members.map(m => m.userId._id));
  const [error, setError] = useState('');

  const editExpense = useMutation({
    mutationFn: data => api.put(`/groups/${groupId}/expenses/${expense._id}`, data),
    onSuccess: () => {
    ...
    onError: err => setError(err.response?.data?.message || 'Failed to update expense'),
  });
  ...
  return (
    <div style={S.overlay}>
      <div style={S.modal}>
        <h3 style={S.title}>Edit Expense</h3>
        <form onSubmit={handleSubmit} style={S.form}>
          ...
          <div style={S.splitSection}>
            <p style={S.splitLabel}>
              Split among:
              {perPerson && <span style={S.perPerson}> ₹{perPerson} each</span>}
            </p>
          ...
          <div style={S.actions}>
            ...
            <button type="submit" style={S.submitBtn} disabled={editExpense.isPending}>
...
const S = {
  ...
  perPerson: { color: '#667eea', fontWeight: 700 },
